  ...
  const columns = [
    {
      ...
      render: (text, staff) => (
        <Space size="middle">
          <Link className="staff-name" to={`/admin/partners/communities/${homeId}/staff/${staff.key}`}>{humanize(text)}</Link>
    ...
    },
    {
      ...
      render: (text, staff) => (
        <Space size="middle">
          <Tooltip title="Edit"><Link to={`/admin/partners/communities/${homeId}/staff/${staff.key}`}><EditOutlined /></Link></Tooltip>
        </Space>
      ),
    },
  ];
      ...
      <Row gutter={30}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24}>
          <Space style={{ marginBottom: 16 }} className="btn-group-right">
            <Button href={`/dashboard/admin/partners/communities/${homeId}/staff/add`}>Add Staff</Button>
          </Space>
          <Table
            columns={columns}
